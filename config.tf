terraform {
  ...
  backend "s3" {
    ...
    # dynamodb_table = "terraform-state-lock-app" # чтобы state файл одновременно редактировал только один пользователь, но dynamodb_table должна быть создана заранее, так как terraform сам его не создаст
  }
}
